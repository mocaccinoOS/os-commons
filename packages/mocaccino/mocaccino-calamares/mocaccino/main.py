...
def run():
    ...
    libcalamares.utils.target_env_call(['dracut', '-q', '-H', '-f','-o','systemd', '-o', 'systemd-initrd', '-o', 'systemd-networkd', '-o', 'dracut-systemd', '--kver', '$(uname -a)', '/boot/initramfs-genkernel-$(uname -m)-$(uname -a)'])
    # Get install path
